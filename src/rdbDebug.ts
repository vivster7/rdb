...
export class RDBDebugSession extends LoggingDebugSession {
  ...
  public constructor(fileAccessor: FileAccessor) {
    ...
    this._runtime.on("stopOnDataBreakpoint", () => {
      this.sendEvent(
        new StoppedEvent("data breakpoint", RDBDebugSession.threadID)
      );
    });
    this._runtime.on("stopOnException", (exception) => {
      if (exception) {
        this.sendEvent(
          new StoppedEvent(`exception(${exception})`, RDBDebugSession.threadID)
        );
      } else {
    ...
    this._runtime.on("rawEvent", (event: DebugProtocol.Event) => {
  ...
  protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
    ...
    response.body = {
      threads: [new Thread(RDBDebugSession.threadID, "thread 1")],
    };
    this.sendResponse(response);
  }
