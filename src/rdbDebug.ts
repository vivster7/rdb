 ...
 * The schema for these attributes lives in the package.json of the rdb-debug extension.
...
export class RDBDebugSession extends LoggingDebugSession {

	// we don't support multiple threads, so we can use a hardcoded ID for the default thread
	private static threadID = 1;

	// a RDB runtime (or debugger)
	private _runtime: RDBRuntime;
	...
	public constructor(fileAccessor: FileAccessor) {
		...
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', RDBDebugSession.threadID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', RDBDebugSession.threadID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', RDBDebugSession.threadID));
		});
		this._runtime.on('stopOnDataBreakpoint', () => {
			this.sendEvent(new StoppedEvent('data breakpoint', RDBDebugSession.threadID));
		});
		this._runtime.on('stopOnException', (exception) => {
			if (exception) {
				this.sendEvent(new StoppedEvent(`exception(${exception})`, RDBDebugSession.threadID));
			} else {
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(RDBDebugSession.threadID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
	...
	// if (ds) {
	// 	ds.customRequest('customRequest');
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'rdb-adapter-data');
	}
}
